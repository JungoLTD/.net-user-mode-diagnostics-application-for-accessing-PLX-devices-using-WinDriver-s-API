...
namespace Jungo.PLX_Sample
{
    public class AddrSpaceTransferForm : System.Windows.Forms.Form
    {
        ...
        public AddrSpaceTransferForm(PLX_Device dev, string[] sBars,
            bool bIsLocal)
        {
            InitializeComponent();
            ...
            this.Text = "Read/Write Address Space" +
                (m_bIsLocal? " through Local Bus" : "") + " Form";
...
#region Windows Form Designer generated code
        ...
        private void ReadWriteLocalBus()
        {
            DWORD dwStatus = 0;
            BOOL bIsBlock = (m_type == TRANSFER_TYPE.BLOCK);
            BOOL bIsRead = (m_direction == RW.READ);
            ...
            case WDC_ADDR_MODE.WDC_MODE_8:
                {
                    if (bIsRead)
                        dwStatus = bIsBlock ?
                            m_device.ReadAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_bData, m_mode,
                                dwOptions) :
                            m_device.ReadAddrLocal8((PLX_ADDR)m_dwBar,
                                m_dwOffset, ref m_bData[0]);
                    else
                        dwStatus = bIsBlock?
                            m_device.WriteAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_bData, m_mode,
                                dwOptions) :
                            m_device.WriteAddrLocal8((PLX_ADDR)m_dwBar,
                                m_dwOffset, m_bData[0]);
            ...
            case WDC_ADDR_MODE.WDC_MODE_16:
                {
                    if (bIsRead)
                        dwStatus = bIsBlock ?
                            m_device.ReadAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_wData, m_mode,
                                dwOptions) :
                            m_device.ReadAddrLocal16((PLX_ADDR)m_dwBar,
                                m_dwOffset, ref m_wData[0]);
                    else
                        dwStatus = bIsBlock?
                            m_device.WriteAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_wData, m_mode,
                                dwOptions) :
                            m_device.WriteAddrLocal16((PLX_ADDR)m_dwBar,
                                m_dwOffset, m_wData[0]);
            ...
            case WDC_ADDR_MODE.WDC_MODE_32:
                {
                    if (bIsRead)
                        dwStatus = bIsBlock ?
                            m_device.ReadAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_u32Data, m_mode,
                                dwOptions) :
                            m_device.ReadAddrLocal32((PLX_ADDR)m_dwBar,
                                m_dwOffset, ref m_u32Data[0]);
                    else
                        dwStatus = bIsBlock?
                            m_device.WriteAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_u32Data, m_mode,
                                dwOptions) :
                            m_device.WriteAddrLocal32((PLX_ADDR)m_dwBar,
                                m_dwOffset, m_u32Data[0]);
            ...
            case WDC_ADDR_MODE.WDC_MODE_64:
                {
                    if (bIsRead)
                        dwStatus = bIsBlock ?
                            m_device.ReadAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_u64Data, m_mode,
                                dwOptions) :
                            m_device.ReadAddrLocal64((PLX_ADDR)m_dwBar,
                                m_dwOffset, ref m_u64Data[0]);
                    else
                        dwStatus = bIsBlock?
                            m_device.WriteAddrLocalBlock((PLX_ADDR)m_dwBar,
                                m_dwOffset, dwFloorBytes, m_u64Data, m_mode,
                                dwOptions) :
                            m_device.WriteAddrLocal64((PLX_ADDR)m_dwBar,
                                m_dwOffset, m_u64Data[0]);
        ...
        private void TranslateInput()
        {
            ...
            if ((uint)cmboBar.SelectedIndex == 0xffffffff)
                throw m_excp;
            m_dwBar = (DWORD)cmboBar.SelectedIndex + (DWORD)(m_bIsLocal?
                PLX_ADDR.PLX_ADDR_SPACE0 : PLX_ADDR.PLX_ADDR_REG);
            ...
            if (txtNumBytes.Enabled == true)
            {
                ...
                m_excp = new Exception("Please enter the number of bytes. " +
                    ...
                    m_device.AddrDesc[iBarIndex].qwBytes.ToString("X") + ")");
                m_dwBytes = Convert.ToUInt32(txtNumBytes.Text,16);
                if (m_dwBytes > m_device.AddrDesc[iBarIndex].qwBytes)
                    throw m_excp;
            }
            else
            {
